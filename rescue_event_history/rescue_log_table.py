...
dstr2date = lambda dstr: datetime.date(
    2000 + int(dstr[:2]), int(dstr[2:4]), int(dstr[4:])
)
...
with open("doll_obtain_info.tsv", encoding="utf-8") as f:
    for row in csv.reader(f, delimiter="\t"):
        ...
        for name in dolls:
            rescue_log.setdefault(
                name,
                {
                    "first": {"event": "", "time": dstr2date("300101")},
                    "last": {"event": "", "time": dstr2date("160520")},
                    "info": gun_info.get(name, dict(name=name, id=0, rank=1)),
                },
            )
...
rescue_log = sorted(
    ...
    key=lambda x: (
        x["last"]["time"]
        if x["last"]["time"] != dstr2date("300101")
...
styles = [
    {
        "selector": "",
        "props": "margin-left: auto; margin-right: auto; text-align: center;padding: 0.5em 1em;width:72em",
    },
    {"selector": "th,td", "props": "padding: 0.5em 1em;"},
    {
        "selector": "thead",
        "props": "background-color: #EEE; color: #333;display: block;",
    },
    {
        "selector": "tbody",
        "props": "color: white;display: block;overflow: auto;width: 100%;height: 90vh;",
    },
...
width = [
    {"selector": f"td:nth-child({c+1}),th:nth-child({c+1})", "props": f"width: {w}em;"}
    for c, w in enumerate([2, 15, 3, 10, 10, 10, 10, 10])
]
